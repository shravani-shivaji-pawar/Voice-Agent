...
VALID_TTS_PROVIDERS = {"edge", "cartesia", "parler", "elevenlabs"}
...
class AgentCreate(BaseModel):
    ...
    elevenlabs_similarity: Optional[float] = None
...
async def get_elevenlabs_voices():
    ...
    try:
        ...
        for voice in res.voices:
            gender = voice.labels.get("gender") or voice.labels.get("Gender") or "unknown"
            accent = voice.labels.get("accent") or voice.labels.get("Accent") or "unknown"
            private_voices.append({
                "name": voice.name,
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as http_client:
                r = await http_client.get(
                    ...
                    headers=headers,
                    params={"accent": "indian", "page_size": 100}
                )
                if r.status_code == 200:
                    voices_list = r.json().get("voices") or []
                    for v in voices_list:
                        shared_indian.append({
                            "name": v.get("name") or "Unnamed",
                            "voice_id": v["voice_id"],
                            "gender": v.get("gender") or "unknown",
                            "accent": v.get("accent") or "indian",
                            "description": v.get("description") or "",
                            "category": "shared",
                            "preview_url": v.get("preview_url") or ""
                        })
        except Exception as e:
            logger.error("Failed to fetch ElevenLabs shared Indian voices: %s", e)

        tier = "free"
        try:
            async with httpx.AsyncClient(timeout=4.0) as http_client:
                r = await http_client.get("https://api.elevenlabs.io/v1/user/subscription", headers=headers)
                if r.status_code == 200:
                    sub_data = r.json()
                    tier = sub_data.get("tier") or "free"
        except Exception as e:
            logger.error("Failed to fetch ElevenLabs subscription info: %s", e)

        has_library_access = (tier.lower() != "free")

        indian_voices = list(shared_indian)
        indian_keywords = {
            "indian", "india", "hindi", "marathi", "bengali", "telugu", "tamil", "urdu", "kannada", 
            "gujarati", "punjabi", "malayalam", "odia", "assamese", "hinglish", "kanika", "ruchi", 
            "priya", "neha", "naveen", "killi", "aditi", "aravind", "deepika", "harish", "madhav", 
            "nisha", "sanjay", "swara", "vihaan", "pari", "indic"
        }
        for pv in private_voices:
            name_lower = str(pv.get("name") or "").lower()
            accent_lower = str(pv.get("accent") or "").lower()
            desc_lower = str(pv.get("description") or "").lower()
            is_indian_name = any(kw in name_lower for kw in indian_keywords)
            is_indian_label = any(kw in accent_lower for kw in indian_keywords) or any(kw in desc_lower for kw in indian_keywords)
            if is_indian_label or is_indian_name:
                if not any(v["voice_id"] == pv["voice_id"] for v in indian_voices):
                    indian_voices.append(pv)

        all_voices = list(private_voices)
        for v in shared_indian:
            if not any(pv["voice_id"] == v["voice_id"] for pv in all_voices):
                all_voices.append(v)
        ...
        return {
            ...
            "tier": tier,
            "has_library_access": has_library_access
        }
    except Exception as e:
...
async def get_elevenlabs_models():
    ...
    if not api_key:
        return [
            {"model_id": "eleven_flash_v2_5", "name": "Eleven Flash v2.5 (Recommended for speed)"},
            {"model_id": "eleven_multilingual_v2", "name": "Eleven Multilingual v2 (Premium Quality)"},
...
def _normalize_agent_record(data: dict) -> dict:
    ...
    normalized["elevenlabs_voice_id"] = str(normalized.get("elevenlabs_voice_id") or "").strip()
    normalized["elevenlabs_model"] = str(normalized.get("elevenlabs_model") or "eleven_flash_v2_5").strip()
    normalized["elevenlabs_stability"] = float(normalized.get("elevenlabs_stability") if normalized.get("elevenlabs_stability") is not None else 0.5)
    normalized["elevenlabs_similarity"] = float(normalized.get("elevenlabs_similarity") if normalized.get("elevenlabs_similarity") is not None else 0.75)
    normalized["elevenlabs_style"] = float(normalized.get("elevenlabs_style") if normalized.get("elevenlabs_style") is not None else 0.0)
    normalized["elevenlabs_speaker_boost"] = bool(normalized.get("elevenlabs_speaker_boost") if normalized.get("elevenlabs_speaker_boost") is not None else True)
...
def _write_agent_runtime_schema(
    ...
    schema["provider_config"] = {
        ...
        "elevenlabs_voice_id": agent_data.get("elevenlabs_voice_id"),
        "elevenlabs_model": agent_data.get("elevenlabs_model"),
        "elevenlabs_stability": agent_data.get("elevenlabs_stability"),
        "elevenlabs_similarity": agent_data.get("elevenlabs_similarity"),
        "elevenlabs_style": agent_data.get("elevenlabs_style"),
        "elevenlabs_speaker_boost": agent_data.get("elevenlabs_speaker_boost"),
    }
