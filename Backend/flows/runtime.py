...
try:
    from pipecat.frames.frames import AudioRawFrame, Frame, TextFrame, CancelFrame, EndFrame, TTSAudioRawFrame
...
class RealEstateLLMProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection = None):  # type: ignore
        ...
        if isinstance(frame, StartFrame) and not self._booted:
            ...
            # Sync session and LLM language with agent's schema configuration
            schema_lang = str(self.state_manager.schema.get("language") or "").lower().strip()
            if "hindi" in schema_lang or schema_lang == "hi":
                self.current_language = "hi"
            elif "marathi" in schema_lang or schema_lang == "mr":
                self.current_language = "mr"
            elif "hinglish" in schema_lang:
                self.current_language = "hinglish"
            else:
                self.current_language = "en"
            self.state_manager.set_active_language(self.current_language)
            self.language_tracker.current_language = self.current_language
            logger.info("[PIPELINE] LLM -> Active language set from schema config: %s (value was: %s)", self.current_language, schema_lang)
            ...
            async with _ml_semaphore:
                while True:
                    ...
                    if chunk_bytes:
                        ...
                        out_frame = TTSAudioRawFrame(audio=chunk_bytes, sample_rate=24000, num_channels=1)
                        _ensure_frame_runtime_attrs(out_frame)
